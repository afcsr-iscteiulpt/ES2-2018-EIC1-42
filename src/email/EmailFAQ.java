...
public class EmailFAQ {
	...
	/**
	 * Classe de envio de email de questões ao administrador
	 * 
	 * @param ood1mail
	 * @param password
	 * @param usermail
	...
	public EmailFAQ(String ood1mail, String password, String usermail, String faqSubj, String messagefaq) {
		...
		try {
			...
			message.setRecipients(Message.RecipientType.TO, InternetAddress.parse(ood1mail));
			for (int i = 0; i < CClist.length; i++)
				message.addRecipient(Message.RecipientType.CC, CClist[i]);

			LocalDateTime now = LocalDateTime.now();
			message.setSubject("FAQ Mail: " + FAQSubject + " " + now.getYear() + "-" + now.getMonth() + "-"
